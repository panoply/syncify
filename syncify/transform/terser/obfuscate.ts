...
export function parseClasses (css: string) {
  ...
  if (config.types.size > before) {
    const plur = config.types.size - before;
    log(plur > 1
      ? `${plur} class selectors were added to mcss.d.ts`
      : `${plur} class selector was added to mcss.d.ts`);

  }

  if (remove.size > 0) {
    const plur = remove.size;
    warn(
      plur > 1
        ? `${plur} class selectors were removed from mcss.d.ts`
        : `${plur} class selector was removed from mcss.d.ts`
    );
