...
export async function injectSnippet () {
  ...
  log.update(tree.Line(gray(` ${TLD} ${neonCyan(key)} uploaded snippet injection`)));
...
export async function injectRender (path: string) {
  ...
  if (!EXP.test(content)) {
    ...
    log.update(tree.Line(gray(` ${TLD} injected render tag in output layout`)));
  }
  ...
  if (isString(string)) {
    ...
    if (upload) {
      log.update(tree.Line(gray(` ${TLD} uploaded and inject render tag`)));
      return true;
    }
