/**
 * Web Component Patches
 *
 * Majority of this is for Dawn themes and a reminder that Dawn is an abomination.
 * We are monkey patching native methods to ensure HOT Swaps persist.
 */
  ...
  customElements.define = function (name: string, constructor: CustomElementConstructor, options?: any) {
    ...
    if (WebC.has(name)) {
      ...
      document.body.querySelectorAll(oldName).forEach(oldElement => {
        ...
        console.info(`HOT: Web Component '${name}' was refined to ${newName} (refresh to reset)`);
      ...
      console.warn(`HOT: Web component '${name}' will be monkey patched during HOT Mode.`);
