...
module.exports = function *(config) {
  ...
  while (choice.action !== 'Done managing targets') {
    ...
    let targetChoices = config.targets.map(function (target) {
      return `[${target.target_name}] - '${target.theme_name}' at ${target.shopify_domain}.myshopify.com`
    })
    ...
    if (['Create target', 'Edit target'].includes(choice.action)) {
      ...
      let targetSettings = yield inquirer.prompt([
        ...
        },
        {
          ...
          default: (currTarget.shopify_domain || null)
        },
        {
          type: 'input',
          name: 'store_domain',
          message: 'Store Domain? (eg: https://www.domain.com)',
          default: (currTarget.store_domain|| null)
        },
      ])

      currTarget = Object.assign(currTarget, targetSettings)
      currTarget.shopify_domain = currTarget.shopify_domain.replace(new RegExp('^https?://'), '').replace(new RegExp('\.myshopify\.com.*'), '')
