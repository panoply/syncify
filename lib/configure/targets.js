...
module.exports = function *(config) {
  ...
  while (choice.action !== 'Done managing targets') {
    ...
    if (['Create target', 'Edit target'].includes(choice.action)) {
      ...
      currTarget.domain = currTarget.domain.replace(new RegExp('^https?://'), '').replace(new RegExp('\.myshopify\.com.*'), '')
      ...
      currTarget.primary_domain = shop.force_ssl ? `https://${shop.domain}` : `http://${shop.domain}`;
