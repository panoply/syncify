...
export async function hasTemplateMismatch (cwd: string): Promise<Mismatch | Set<string>> {
  ...
  if (exists.size > 1) {
    log.out(tui.suffix('redBright', 'error', `${c.bold(`${exists.size}`)} mismatch template files`));
  } else {
    log.out(tui.suffix('redBright', 'error', `${c.bold(`${exists.size}`)} mismatch template file`));
  }
