...
let storeignore = `
# This your '.storeignore' file. Anything you put in here will be ignored by shopify-sync. This file uses the same format as a '.gitignore' file.
`
...
module.exports = function *(argv) {
  ...
  try {
    ...
    if (config.ignore_file === '.gitignore') {
      ...
      yield fs.writeFileAsync(path.join(process.cwd(), config.ignore_file), storeignore)
    }
  }

  config.configVersion = CONFIGVERSION
  yield fs.writeFileAsync('store.config.json', JSON.stringify(config))
