...
let run = function (target, request) {
  // console.log(target)
  let queue
  if (queues[target.shopify_domain]) {
    queue = queues[target.shopify_domain]
  } else {
    queue = createQueue()
    queues[target.shopify_domain] = queue
  }
...
let url = function (target) {
  return `https://${target.api_key}:${target.password}@${target.shopify_domain}.myshopify.com`
}
