...
async function promptOverwrite (remote: Requests.Page): Promise<{
  /**
   * Resumes the log and reopen log group
   */
  resume: () => void;
...
export async function compile (file: File, _cb: Syncify) {
  ...
  if (isObject(remote)) {
    ...
    if (!(remote.id in $.cache.pages?.[store.domain])) {
      await cache.pages(store.domain, remote);
      cached = remote;
    } else {
  ...
  await cache.pages(store.domain, update);
