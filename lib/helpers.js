...
var loadConfig = function *() {
  ...
  try {
    ...
    throw new Error('Shop configuration is corrupt, you may need to delete \'sync.config.json\', and run \'store configure\' again.')
  }
...
var getTarget = function *(config, argv) {
  ...
  if (_.isArray(config.targets)) {
    if (targetName) {
      ...
      var targetChoices = _.map(config.targets, (target) => { return `[${target.target_name}] - '${target.theme_name}' at ${target.shopify_domain}.myshopify.com` })
