...
Syncify offers robust JavaScript bundling capabilities, tree-shaking, code splitting, and minification built directly into its core utilizing [ESBuild](https://esbuild.github.io/). The `script` transform configuration option allows developers to process JavaScript or TypeScript efficiently and comes with compatible presets tailored for producing the most optimal distribution bundles for Shopify themes.
...
# Using jsconfig or tsconfig files
