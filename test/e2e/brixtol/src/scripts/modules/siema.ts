...
export class Siema {
  ...
  private init (): void {
    ...
    this.selector.style.willChange = 'auto';
    // rtl or ltr
  ...
  private attachEvents (): void {
    ...
    if (this.config.draggable) {
      ...
      this.selector.addEventListener('mouseleave', this.mouseleaveHandler, { passive: true });
  ...
  private buildSliderFrame (): void {
    ...
    if (this.config.loop) {
      ...
      for (; slide < this.innerElements.length; slide++) {
        const element = this.buildSliderFrameItem(this.innerElements[slide].cloneNode(true) as HTMLElement);
