...
import { Paths, Directories, Live, Transforms, Minify, Views } from '../config/index';
...
export interface Modes {
  ...
  export: boolean;
  /**
   * Whether or not live reload should be invoked: `--live`
   */
  live: boolean;
}
...
export interface Bundle {
  ...
  svg: {
    icons: Views.Icons,
    sprite: Merge<Transforms.SVGSprite, { input: string[]; }>[];
    inline: Merge<Transforms.SVGInline, { input: Tester; }>[]
  };
  /**
   * JSON handling
   */
  json: Merge<Transforms.JSON, { exclude: Tester }>;
  /**
   * Minify Options
   */
  minify: {
