...
import { IBundle, IOptions, ITerser, ITransform } from 'types';
 ...
 * we can reference it.
 */
export const defaults = mergeDeepRight<IOptions>(<IOptions>{
  input: 'source',
  output: 'theme',
  import: 'import',
  export: 'export',
 ...
 */
export let transform: ITransform = ({
  styles: [],
  json: {
    indent: 2,
    useTabs: false,
...
export const update = ({
  ...
  terser: (patch: PartialDeep<ITerser>) => {
    terser = merge(
      terser,
      patch
    );
  },
  transform: (patch: PartialDeep<ITransform>) => {
    transform = merge(
      transform,
      patch
    );
