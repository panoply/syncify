...
async function getEnvFile(cwd) {
  ...
  if (await fsExtra.pathExists(path2)) {
    const env3 = dotenv__default.default.config({ path: path2 });
    if (env3.error) {
...
function setStores(cli) {
  ...
  if (hasPath("syncify.stores", $.pkg)) {
    ...
    } else if (isEmpty2($.pkg.syncify.stores) === false) {
      array = [$.pkg.syncify.stores];
    }
  } else if ($.config.stores !== null) {
    array = isArray2($.config.stores) ? $.config.stores : [$.config.stores];
  }
  if (cli._.length === 0) {
  ...
  for (const store of items) {
    ...
    if ($.mode.metafields || $.mode.pages)
      return;
    const themes2 = has2("theme", cli) ? cli.theme.split(",") : has2(store.domain, cli) ? cli[store.domain].split(",") : keys(store.themes);
