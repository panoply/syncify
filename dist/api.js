...
`${gray.open}-${gray.close}`;
`${gray.open},${gray.close}`;
`${neonGreen.open}\u2713${neonGreen.close}`;
...
function invalidCommand({
  ...
  error(
    Create({ type: "error" }).Line("COMMAND ERROR", bold).NL.Wrap(message).NL.Line(`provided${COL} ${whiteBright("$")} ${whiteBright("syncify " + provided)}`).Line(`expected${COL} ${whiteBright("$")} ${expected}`).NL.Line("How to fix?", gray.bold).Wrap(fix, gray).End($.log.group).BR.toString()
  );
  process.exit(0);
}
...
runtime.time = function() {
  log(
    NextLine(lightGray(`Started in ${timer.stop("runtime")}`))
  );
};
...
function upload(theme2) {
  ...
  if ($.mode.watch) {
    ...
    log(
      Line(
        neonGreen(
          Prefix("uploaded", bold(theme2.target), theme2.store, timer.stop())
        )
      )
    );
  }
...
function retrying(file, theme2) {
  log(
    Line(
      orange(
        Prefix("retrying", file, theme2.target, theme2.store)
      )
    )
  );
}
function deleted(file, theme2) {
  log(
    Line(
      blueBright(
        Prefix("deleted", file, theme2.target, theme2.store)
      )
    )
  );
}
...
async function injectSnippet() {
  ...
  const [theme2] = $.sync.themes;
  const snippet2 = await fsExtra.readFile($.hot.snippet);
  const upload4 = await upload2(snippet2.toString(), { theme: theme2, key });
...
async function ejectRender(path2) {
  ...
  if (isString(string)) {
    if (EXP.test(string)) {
      ...
      const removed = await upload2(content, { theme: theme2, key });
      return removed;
    }
    return true;
  }
...
function getModel(size2) {
  ...
  for (const theme2 of $.sync.themes) {
    if (theme2.target.length > width)
      width = theme2.target.length;
...
async function upload3(cb) {
  ...
  for (const {
    ...
    if (errors.remote.size > 0) {
      const name = bold(`${theme2.target.toUpperCase()} THEME`);
      const failures = bold(`${failed}`);
      const uploaded = `${bold(`${success}`)} ${white("of")} ${bold(`${size2}`)}`;
      log(
        Create().Line(`${name}  ${ARR}  ${theme2.store}`).NL.Line(Prefix("uploaded", uploaded), neonGreen).Line(Prefix("failures", failures), redBright).NL.toString()
      );
      let number = 1;
...
async function ParseSharedSchema(file) {
  try {
    ...
    if (has("$schema", schema3))
      delete schema3.$schema;
    if (has("$description", schema3))
...
async function compile4(file, cb) {
  ...
  if (isBuffer(json2)) {
    ...
    if (file.type === 7 /* Config */ && file.name === "settings_data") {
      for (const theme2 of $.sync.themes) {
        await find("config/settings_data.json", theme2);
      }
    }
...
async function postcssProcess(file, css, map) {
  ...
  try {
    if ($.mode.watch)
      timer.start();
    const plugins2 = isBoolean(file.data.postcss) ? $.processor.postcss.config : file.data.postcss;
...
async function load2(id) {
  if (id === "svg-sprite") {
    ...
    return isNil2(SVGSprite) === false;
  }
  if (id === "svgo") {
    Svgo = (await import('svgo')).default;
    return isNil2(Svgo) === false;
  }
}
...
async function promptAction(store) {
  ...
  const prompt3 = await prompts2__default.default({
    type: "select",
    name: "action",
...
async function compile8(file, _cb) {
  ...
  if (isUndefined(remote)) {
    if ($.page.safeSync) {
      const prompt3 = await promptAction(store);
      if (prompt3.action === 2 /* Select */) {
        const action = await selectPage(store);
        if (action === 4 /* Cancel */) {
          return prompt3.resume();
      ...
      } else if (prompt3.action === 1 /* Create */) {
        prompt3.resume();
        syncing(`/pages/${payload.handle} ${ARR} ${payload.title} ${gray(`${TLD} ${file.relative}`)}`);
        return create4(store, payload);
      } else {
  ...
  if ($.page.safeSync && isObject(remote)) {
    ...
    if (online > local && remote.body_html !== payload.body_html) {
      const prompt3 = await promptOverwrite(remote);
      if (prompt3.action === 3 /* Update */) {
        prompt3.resume();
      ...
      } else if (prompt3.action === 4 /* Cancel */) {
        return prompt3.resume();
      } else if (prompt3.action === 6 /* Overwrite */) {
        prompt3.resume();
      }
    }
  }
...
async function importing(cb) {
  ...
  function callback(item) {
    ...
    const prefix = Create().NL.Line(Prefix("Duration", whiteBright(timer.now("import"))), gray).Line(Prefix("Transfers", whiteBright(`${transfers++}`)), gray).Line(Prefix("Syncing", pink(`${bold(theme2.target)}  ${ARR}  ${theme2.store}`)), gray).Line(Prefix("Preview", underline(preview)), gray).Ruler();
    let processing = "";
...
async function getEnvFile(cwd) {
  let path2 = pathe.join(cwd, ".env");
  if (await fsExtra.pathExists(path2)) {
    ...
    path2 = pathe.join(cwd, "syncify.env");
    if (await fsExtra.pathExists(path2)) {
      const env2 = dotenv__default.default.config({ path: path2 });
      if (env2.error) {
...
async function injection() {
  log_update_default(Line(gray(` ${TLD} validating snippet injection`)));
  const snippet2 = await injectSnippet();
  if (snippet2) {
    log_update_default(Line(gray(` ${TLD} validating layouts`)));
    for (const layout in $.hot.alive) {
      const exists2 = await fsExtra.pathExists(layout);
      if (!exists2) {
        log_update_default(Line(gray(` ${TLD} layout has not yet been bundled, building now...`)));
        ...
        log_update_default(Line(gray(` ${TLD} layout was bundled from source, injecting hot snippet`)));
      }
      try {
...
async function server() {
  ...
  log_update_default(Line(` ${TLD} configuring HOT Reload`));
  await injection();
  ...
  log_update_default(Line(` ${TLD} ${neonCyan("server")}  ${ARR}  ${port}`));
  log(Line(` ${TLD} ${neonCyan("socket")}  ${ARR}  ${sock}`));
  for (const p in $.hot.alive) {
    log(Line(` ${TLD} ${neonCyan("layout")}  ${ARR}  ${gray(pathe.relative($.cwd, p))}`));
  }
  nwl();
}
function socket() {
...
function setJsonOptions() {
  ...
  if (isNil(json2))
    return;
  if (isObject(json2) && isEmpty(json2))
    return;
  if (!isObject(json2)) {
    typeError(
      {
...
function setStores(cli) {
  ...
  for (const store of items) {
    ...
    if ($.mode.metafields || $.mode.pages)
      return;
    const themes2 = has("theme", cli) ? cli.theme.split(",") : has(store.domain, cli) ? cli[store.domain].split(",") : keys(store.themes);
...
async function setScriptOptions() {
  if (!has("script", $.config.transform))
    return;
  if (!$.config.transform.script || isEmpty($.config.transform.script))
    return;
  ...
  for (const script3 of transforms) {
    ...
    if (script3.snippet) {
      ...
      if (has3("attrs") && isEmpty(script3.attrs) === false) {
        if (isArray(script3.attrs)) {
    ...
    if (has3("esbuild")) {
      if (isBoolean(script3.esbuild) || isNil(script3.esbuild)) {
        ...
        if (esProp("plugins") && has("plugins", esbuild4.config)) {
...
async function setStyleConfig() {
  if (!has("style", $.config.transform))
    return;
  if (!$.config.transform.style || isEmpty($.config.transform.style))
    return;
    ...
    if (has3("postcss")) {
      ...
      if (isArray(style2.postcss)) {
        ...
        if (isBoolean(style2.postcss) || override) {
          if (style2.postcss !== false && isNil(style2.postcss) === false) {
            if (!postcss4.installed)
              missingDependency("postcss");
...
async function setSvgOptions() {
  if (!has("svg", $.config.transform))
    return;
  if (!$.config.transform.svg || isEmpty($.config.transform.svg))
    return;
...
async function setHotReloads() {
  ...
  if (isObject($.config.hot) && isEmpty($.config.hot) === false) {
    ...
    for (const prop in $.config.hot) {
      ...
      } else if (typeof hot2[prop] === typeof $.config.hot[prop]) {
        hot2[prop] = $.config.hot[prop];
      } else {
        typeError({
          option: "hot",
