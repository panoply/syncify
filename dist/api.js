...
function errorRuntime(e2, options) {
  ...
  if (has2("code", e2)) options.entries.code = e2.code;
  if (has2("name", e2)) options.entries.name = e2.name;
  error(
...
function normalPath(input, cwd = null) {
  ...
  return function prepend(path2) {
    if (isArray(path2)) return path2.map(prepend);
...
async function compile3(file, sync4, cb) {
  ...
  try {
    ...
    if (isNil(import_postcss.default) || !file.data.postcss && !file.data.snippet) {
...
function setJsonOptions() {
  ...
  if (isObject(json2) && isEmpty(json2)) return;
  if (!isObject(json2)) {
    typeError(
      {
        option: "processors",
...
async function setPaths() {
  ...
  for (const key of PATH_KEYS) {
    let uri;
    if (key === "customers") {
      uri = has3(key) ? isArray($.config.paths[key]) ? $.config.paths[key].map(path2) : [path2($.config.paths[key])] : [path2("templates/customers/*")];
    } else if (key === "metaobject") {
      uri = has3(key) ? isArray($.config.paths[key]) ? $.config.paths[key].map(path2) : [path2($.config.paths[key])] : [path2("templates/metaobject/*")];
    } else if (key === "schema") {
      uri = has3(key) ? isArray($.config.paths[key]) ? $.config.paths[key].map(path2) : [path2($.config.paths[key])] : [path2("schema/*")];
    } else if (has3(key)) {
  ...
  if (typeof $.config.terser === "object") {
    for (const key in $.config.terser) {
      ...
      if (isBoolean($.config.terser[key])) {
        ...
        if (isEmpty(terser2[key])) continue;
        for (const opt in $.config.terser[key]) {
          ...
          if (!has(opt, terser2[key])) unknownError(`terser.${key}`, opt);
          if (opt === "mangleProps") {
