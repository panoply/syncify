...
function Modes(mode) {
  ...
  const write = syncify_js.Create().Top(`Syncify ${syncify_js.CHV} Error`, false).Header("NOT YET AVAILABLE", syncify_js.yellowBright2.bold);
  if (syncify_js.COMMAND_MODES.has(mode)) {
    ...
    write.Wrap(message, syncify_js.yellowBright2).Newline().Each(modes, (name) => name in Modes ? write.Line(`$ sy help ${name} ${equal(name) + syncify_js.CHK}`, syncify_js.whiteBright2) : write.Line(`$ ${syncify_js.strikethrough2(`sy help ${name}`)}`, syncify_js.gray2));
  }
  write.NL.End(`Syncify ${syncify_js.CHV} Error`, false).BR.toLog(highlight);
}
Modes.init = () => {
  const info = description("init");
  syncify_js.Create().BR.Top(`help ${syncify_js.TLD} sy inspect`, false).True(info, (tui) => tui.Header(`DESCRIPTION${syncify_js.COL}`, syncify_js.bold2).Wrap(syncify_js.gray2, info.overview)).True(info, (tui) => tui.Prepend(info.reference, syncify_js.gray2.underline)).Header(`COMMANDS${syncify_js.COL}`, syncify_js.bold2).Line(" $ sy init").NL.End(`help ${syncify_js.TLD} sy inspect`, false).BR.toLog(highlight);
};
Modes.push = () => {
    ...
    if ("flags" in mode) {
      ...
      while (++i2 < s) {
        if (mode.flags[i2] in flag) {
          ...
          CommandError([
            `Unknown flag expression provided "${syncify_js.bold2(`--${mode.flags[i2]}`)}"
`,
...
`,
    ...
    try {
      ...
      CommandError(error.message.replace(/(--?)([a-z-]+)?/g, syncify_js.red2.bold("$1$2")));
    }
  }
}
...
function command(commands) {
  ...
  if (syncify_js.$.mode.build) {
    ...
    if (syncify_js.isNull(syncify_js.$.log.mode) && syncify_js.$.mode.push) syncify_js.$.log.mode = 7 /* Push */;
    if (syncify_js.isNull(syncify_js.$.log.mode) && syncify_js.$.mode.pull) syncify_js.$.log.mode = 8 /* Pull */;
    if (syncify_js.isNull(syncify_js.$.log.mode) && syncify_js.$.mode.build) syncify_js.$.log.mode = 5 /* Build */;
    if (syncify_js.isNull(syncify_js.$.log.mode) && syncify_js.$.mode.pack) syncify_js.$.log.mode = 9 /* Pack */;
  }
...
function CommandError(message) {
  syncify_js.Create({ type: "error" }).Top(`Syncify ${syncify_js.CHV} Error`, false).Newline(syncify_js.Tree.trim).Line(`COMMAND LINE ERROR ${syncify_js.BAD}`, syncify_js.bold2).Newline().Wrap(message).Tree("info").NL.Line("Need Help?", syncify_js.gray2.bold).Line("Refer to the usage documentation for more information:", syncify_js.gray2).Prepend(`${syncify_js.CHV} ${syncify_js.underline2("https://syncify.sh/usage/syncify-cli")}`, syncify_js.gray2).NL.End(`Syncify ${syncify_js.CHV} Error`, false).BR.toLog();
  syncify_js.$.running ? syncify_js.i.exit(0) : process.exit(0);
}
