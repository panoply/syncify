...
var require_picocolors = __commonJS({
  "node_modules/.pnpm/picocolors@1.1.0/node_modules/picocolors/picocolors.js"(exports, module) {
    ...
    var isColorSupported = !("NO_COLOR" in env2 || argv2.includes("--no-color")) && ("FORCE_COLOR" in env2 || argv2.includes("--color") || process.platform === "win32" || __require != null && __require("tty").isatty(1) && env2.TERM !== "dumb" || "CI" in env2);
...
function authURL(domain) {
  ...
  throwError(
    ...
    [
      `Your shop credentials in the ${import_ansi7.cyan.bold(basename2($.env.file))} file could`,
...
async function compile3(file, sync4, cb) {
  ...
  try {
    ...
    if (isNil2(postcss3) || isUndefined(file.data) || !file.data.postcss && !file.data.snippet) {
...
async function getEnvFile() {
  ...
  if (await pathExists3(path4)) {
    const env2 = dotenv.config({ path: path4 });
    if (env2.error) {
...
var env = {
  get dev() {
    return process4.env.SYNCIFY_ENV === "dev";
  },
  get terse() {
    return $.cmd.terse;
  },
  get prod() {
    return process4.env.SYNCIFY_ENV === "prod";
  },
  get watch() {
    return process4.env.SYNCIFY_WATCH === "true";
  }
};
